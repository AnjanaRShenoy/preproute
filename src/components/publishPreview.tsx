...
import { Calendar, Clock, Award, CheckCircle2, ArrowLeft, Edit2, ChevronDown } from 'lucide-react';
...
export default function PublishPreviewWorkspace({ testDetails, onBack }: PreviewProps) {
  ...
  const handleFinalPublish = async () => {
    ...
    try {
      // Build request payloads dynamically depending on active workspace mode
      const payload: any = {
        status: publishMode === 'schedule' ? 'scheduled' : 'live'
      };

      if (publishMode === 'schedule') {
        payload.scheduled_date = `${scheduleDate}T${scheduleTime}:00.000Z`;
      }
      if (availability === 'custom') {
        payload.expiry_date = `${expiryDate}T${expiryTime}:00.000Z`;
      }
  ...
  return (
    <div className="flex h-screen w-screen bg-slate-50 text-slate-700 font-sans overflow-hidden">
      ...
      {/* SIDEBAR NAVIGATION PLACEHOLDER BLOCKS */}
      <aside className="w-16 bg-white border-r border-slate-100 flex flex-col items-center py-4 justify-between fixed h-full z-20">
        <div className="flex flex-col items-center gap-6 w-full">
          <nav className="flex flex-col items-center gap-2 w-full px-2">
            <div className="p-2.5 text-slate-400"><Calendar size={18} /></div>
            <div className="p-2.5 bg-blue-50 text-blue-600 rounded-lg"><CheckCircle2 size={18} /></div>
          </nav>
        </div>
      </aside>

      <div className="flex-1 flex flex-col ml-16 h-full overflow-y-auto">
        <Header testType="Test Finalization Workspace" />

        <main className="p-8 max-w-5xl w-full mx-auto space-y-8 pb-16">
          
          {/* Breadcrumb Row Context Header Indicators */}
          <div className="flex items-center justify-between">
            ...
            <span className="text-[11px] bg-emerald-50 text-emerald-600 border border-emerald-100 font-black tracking-wider uppercase px-2.5 py-1 rounded-md flex items-center gap-1.5 select-none">
          ...
          <div className="bg-white border border-slate-200/70 shadow-sm rounded-2xl p-6 relative">
            <button type="button" onClick={onBack} className="absolute right-6 top-6 text-slate-300 hover:text-blue-500 transition">
              <Edit2 size={16}/>
            </button>
            <div className="space-y-4">
              ...
              <h2 className="text-xl font-bold text-slate-800 tracking-tight">{testDetails.testName || "Untitled Assignment"}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-y-2 text-xs font-medium text-slate-400 pt-1">
                ...
                <div>Subtopic: <span className="text-slate-700 font-semibold">{testDetails.subTopicName}</span></div>
              </div>
              
              <div className="border-t border-slate-100 pt-4 flex gap-6 text-xs text-slate-500 font-bold select-none">
          ...
          <div className="flex gap-2 border-b border-slate-200 select-none">
            <button 
              type="button"
              onClick={() => setPublishMode('now')} 
              className={`pb-3 text-sm font-bold transition-all relative ${publishMode === 'now' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Publish Now
            </button>
            <button 
              ...
              className={`pb-3 text-sm font-bold ml-6 transition-all relative ${publishMode === 'schedule' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-400 hover:text-slate-600'}`}
            >
              Schedule Publish
            </button>
          </div>

          {/* 👑 3. ADDED: INTERACTIVE SCHEDULE PUBLISH SELECTION FORM GRID BLOCK (FROM image_79f2db.png) */}
          {publishMode === 'schedule' && (
            <div className="space-y-4 animate-fade-in">
              <h3 className="text-sm font-bold text-slate-800">Select Date and Time</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <input 
                    ...
                    className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-xs focus:outline-none focus:border-blue-400 text-slate-600 font-medium transition" 
                  />
                </div>
                <div className="relative">
                  <input 
          ...
          {/* 4. PLATFORM VISIBILITY / DURATION OPTIONS MATRIX SEGMENT */}
          ...
          {availability === 'custom' && (
            <div className="space-y-4 animate-fade-in pt-2">
              <h4 className="text-xs font-bold text-slate-700">Select End Expiry Conditions Threshold</h4>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="relative">
                  <input 
                    type="date" 
          ...
          {/* 5. BOTTOM CONTROL NAVIGATION FOOTER BAR ROW */}
          <div className="flex justify-end gap-4 pt-6 border-t border-slate-200/60">
            <button 
              type="button"
              onClick={onBack} 
              disabled={isSubmitting} 
              className="px-6 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-semibold text-xs rounded-xl transition"
            >
              Cancel
            </button>
            <button 
              ...
              disabled={isSubmitting} 
              className={`px-8 py-2.5 text-white font-semibold text-xs rounded-xl shadow-md transition ${
